...
describe('plugin activation', () => {
  ...
  it('mounts when a third-party plugin fails to apply', async () => {
    const error = vi.spyOn(console, 'error').mockImplementation(() => {})
    const container = document.createElement('div')
    document.body.append(container)
    const target = installFacade()
    const entries: WebBootEntry[] = [
      { id: 'dsh-better-reasoning-effort', url: '/broken.js', rev: '1' },
      { id: 'renderer', url: '/renderer.js', rev: '1' },
    ]
    win.__DSH_BOOT__ = {
      rev: 'graph',
    ...
    const registrations = new Map<string, ClientBundleRegistration>([
      ['/broken.js', {
        ...
        factory: () => ({
          apply: () => {
            throw new Error("Cannot read properties of undefined (reading 'settings')")
          },
        }),
      }],
      ['/renderer.js', {
